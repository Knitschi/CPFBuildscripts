...
class Constants {
    ...
    static final CHECKOUT_FOLDER = 'Cpp CodeBase'
...
if(params.task == 'integration')
{
    ...
    addUpdateWebPageStage( toolchains)
}
    ...
    stage('Create Tmp Branch')
    {
        node('master')
        {
            ws('WS-CppCodeBase')
            {
                ...
                sh "cmake -DDEVELOPER=${developer} -DMAIN_BRANCH=${mainBranch} -DROOT_DIR=\"\$PWD/${CHECKOUT_FOLDER}\" -P \"\$PWD/${CHECKOUT_FOLDER}/Sources/CppCodeBase/Scripts/prepareTmpBranch.cmake\""
            }
        }
    }
}
            ...
            ws('WS-CppCodeBase')
            {
                ...
                dir(CHECKOUT_FOLDER)
                {
                    ...
                    */
                    ...
                    sh "cmake -DDEVELOPER=${developer} -DMAIN_BRANCH=${mainBranch} -DROOT_DIR=\"\$PWD\" -P Sources/CppCodeBase/Scripts/integrateTmpBranch.cmake"
...
def addUpdateWebPageStage(toolchains)
{
    stage('Update Project Web-Page')
    {
        node('master')
        {
            ws('WS-CppCodeBase')
            {
                ...
                sh "cmake -DSOURCE_DIR=\"${tempHtmlDir}\" -DTARGET_DIR=\"${serverHtmlDir}\" -DROOT_DIR=\"\$PWD/${CHECKOUT_FOLDER}\" -P \"\$PWD/${CHECKOUT_FOLDER}/Sources/CppCodeBase/Scripts/updateExistingWebPage.cmake\""
    ...
    stage('Create Release Tag')
    {
        node('master')
        {
            ws('WS-CppCodeBase')
            {
                ...
                sh "cmake -DROOT_DIR=\"\$PWD/${CHECKOUT_FOLDER}\" -DBRANCH=${branch} -DDIGIT_OPTION=${incrementTaskType} -P \"\$PWD/${CHECKOUT_FOLDER}/Sources/CppCodeBase/Scripts/incrementVersionNumber.cmake\""
            }
        }
    }
}
...
def checkoutBranch(branch)
{
    checkout([$class: 'GitSCM',
        userRemoteConfigs: [[url: "ssh://${REPOSITORY_HOST_NAME}/share/GitRepositories/CppCodeBase"]],
        branches: [[name: branch]],
        // We checkout to the subdirectory CppCodeBase so the folders for the test files that lie parallel to the repository are still within the workspace.
        extensions: [[$class: 'CleanBeforeCheckout'],[$class: 'RelativeTargetDirectory', relativeTargetDir: CHECKOUT_FOLDER]]]
    )
}
